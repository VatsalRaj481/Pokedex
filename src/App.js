...
function App() {
  ...
  const [selectedPokemonDescription, setSelectedPokemonDescription] = useState("");
  ...
  // Memoize the list so PokemonScanner doesn't re-render and reset its internal locks
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const pokedexListRes = await axios.get("https://pokeapi.co/api/v2/pokedex");
        const tempPokemonToRegionMap = {};

        const pokedexDetailsPromises = pokedexListRes.data.results.map((pdx) => axios.get(pdx.url));
        const pokedexDetails = await Promise.all(pokedexDetailsPromises);

        pokedexDetails.forEach((pdxDetail) => {
          const regionName = pdxDetail.data.region ? pdxDetail.data.region.name : null;
          if (regionName) {
        ...
        for (const pokemonName in tempPokemonToRegionMap) {
          finalPokemonToRegionMap[pokemonName] = Array.from(tempPokemonToRegionMap[pokemonName]);
        }
  ...
  const fetchAllPokemonData = useCallback(async () => {
    ...
    try {
      ...
      const promises = res.data.results.map((p) => axios.get(p.url).then((r) => r.data));
  ...
  const toSentenceCase = (text) => {
    if (!text) return "";
    return text.charAt(0).toUpperCase() + text.slice(1).toLowerCase().replace(/([.?!]\s*)([a-z])/g, (m, p1, p2) => p1 + p2.toUpperCase());
  };

  const fetchPokemonDescription = useCallback(async (pokemonNameOrId) => {
    try {
      const speciesRes = await axios.get(`https://pokeapi.co/api/v2/pokemon-species/${pokemonNameOrId.toLowerCase()}/`);
      const englishDescription = speciesRes.data.flavor_text_entries.find((entry) => entry.language.name === "en");
      const cleaned = englishDescription ? toSentenceCase(englishDescription.flavor_text.replace(/\f/g, " ")) : "No description available.";
      setSelectedPokemonDescription(cleaned);
    } catch (err) {
      setSelectedPokemonDescription("Description unavailable.");
    }
  }, []);

  const handleSearchAndFilter = useCallback(async (nameQuery, typeQuery, selectedGenerationId) => {
    setIsLoading(true);
    setFilteredPokemon([]);
    ...
    if (!hasFetchedAllPokemon) await fetchAllPokemonData();

    let generationPokemonNames = null;
    if (selectedGenerationId) {
      if (generationSpeciesCache.current[selectedGenerationId]) {
        generationPokemonNames = generationSpeciesCache.current[selectedGenerationId];
      } else {
        const genSpeciesRes = await axios.get(`https://pokeapi.co/api/v2/generation/${selectedGenerationId}/`);
        const speciesNames = new Set(genSpeciesRes.data.pokemon_species.map((s) => s.name));
    ...
    const filtered = pokemonList.filter((p) => {
      const matchesName = nameQuery.trim() ? p.name.toLowerCase().includes(nameQuery.toLowerCase()) : true;
      const matchesType = typeQuery ? p.types.some((t) => t.type.name === typeQuery) : true;
      const matchesGeneration = selectedGenerationId ? generationPokemonNames.has(p.name) : true;
      return matchesName && matchesType && matchesGeneration;
    });

    setFilteredPokemon(filtered);
    if (filtered.length === 1 && filtered[0].name.toLowerCase() === nameQuery.toLowerCase()) {
      setSelectedPokemon(filtered[0]);
      fetchPokemonDescription(filtered[0].name);
      setFilteredPokemon([]);
    }
    setIsLoading(false);
  }, [pokemonList, hasFetchedAllPokemon, fetchAllPokemonData, fetchPokemonDescription]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-100 to-white flex flex-col items-center font-inter">
      ...
      <div className="flex-grow flex flex-col items-center w-full py-8 px-4 sm:px-6 lg:px-8">
        {/* Original Pokeball Title Styling */}
        <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-red-600 to-black drop-shadow-sm mb-8 tracking-tight flex items-center justify-center gap-4">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="80" height="80" className="w-20 h-20 pointer-events-none">
            <circle cx="100" cy="100" r="95" fill="white" stroke="black" strokeWidth="10" />
            <path d="M5,100 a95,95 0 0,1 190,0" fill="red" stroke="black" strokeWidth="10" />
            <rect x="5" y="90" width="190" height="20" fill="black" />
            <circle cx="100" cy="100" r="25" fill="white" stroke="black" strokeWidth="8" />
        ...
        <div className="w-full max-w-4xl flex flex-col md:flex-row items-center justify-center gap-6 mb-12">
          <FilterType types={types} generations={generations} onSearchAndFilter={handleSearchAndFilter} isLoading={isLoading} />
        ...
        {(isLoading || isScanning) && (
          <div className="flex items-center justify-center mt-8">
            ...
            <p className="text-blue-600 text-lg ml-4 font-medium">{isScanning ? "Analyzing image..." : "Loading..."}</p>
          </div>
        )}

        {selectedPokemon && !showScanner && (
          <div className="w-full max-w-5xl mb-8">
            <PokemonCard pokemon={selectedPokemon} layout="horizontal" pokemonToRegionMap={pokemonToRegionMapRef.current} description={selectedPokemonDescription} />
          </div>
        )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-8 w-full max-w-screen-xl">
            {filteredPokemon.map((p) => (
              <div key={p.id} onClick={() => handleCardClick(p)} className="cursor-pointer">
                <PokemonCard pokemon={p} layout="vertical" pokemonToRegionMap={pokemonToRegionMapRef.current} />
              </div>
            ))}
          </div>
        )}

        {!isLoading && !selectedPokemon && filteredPokemon.length === 0 && !isScanning && !showScanner && (
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-8 text-center sm:text-left max-w-2xl">
            <p className="text-xl sm:text-2xl lg:text-3xl text-gray-700 font-medium">Start your Pokémon journey: Search or filter to begin!</p>
            <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/versions/generation-v/black-white/animated/25.gif" alt="Pikachu" className="w-24 h-24 sm:w-32 sm:h-32 object-contain" />
          </div>
        )}
      </div>
      <Footer />
